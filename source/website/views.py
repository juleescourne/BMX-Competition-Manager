...
def add_titulaire() :
    ...
    numero_plaque = (request.form.get('plaqueNb') or "").strip()
    numero_plaque_valide = numero_plaque.isdigit() and int(numero_plaque) < 100
    if numero_plaque_valide:
        numero_plaque = numero_plaque.zfill(2)
        numero_plaque_valide = check_titulaire_number(
            Club.query.filter_by(id=club_id).first().titulaires, numero_plaque
        )

    if not numero_plaque_valide:
        titulaires = Titulaire.query.all()
        clubs = Club.query.all()
        sexes = Sexe.query.all()
            ...
            if not categorie.quart_finie and not categorie.quart_genere :
                if nb_participants_categorie >= 33 :
                    ...
                    for participant in quart_race_a.participations :
                        db.session.add(Participant_manche(titulaire_id=participant.titulaire_id, manche_id=quart_race_a.manches[0].id, place_depart=participant.couloir.couloir_1))
                    for participant in quart_race_b.participations :
                        db.session.add(Participant_manche(titulaire_id=participant.titulaire_id, manche_id=quart_race_b.manches[0].id, place_depart=participant.couloir.couloir_1))
                    for participant in quart_race_c.participations :
                        db.session.add(Participant_manche(titulaire_id=participant.titulaire_id, manche_id=quart_race_c.manches[0].id, place_depart=participant.couloir.couloir_1))
                    for participant in quart_race_d.participations :
                        db.session.add(Participant_manche(titulaire_id=participant.titulaire_id, manche_id=quart_race_d.manches[0].id, place_depart=participant.couloir.couloir_1))
                ...
                if nb_participants_categorie >= 17 :
                    if nb_participants_categorie >= 17 and nb_participants_categorie <= 24 :
                    ...
                    elif nb_participants_categorie >= 25 and nb_participants_categorie <= 32 :
                ...
                if nb_participants_categorie >= 9 and nb_participants_categorie <= 16 :
                    ...
                    for race in pool_races :
                        for participant in race.participations[4:] :
                    ...
                    for demi_race in (demi_race_a, demi_race_b):
                        for index_participant, participant in enumerate(demi_race.participations):
                            if index_participant < 4:
                                db.session.add(Participant_race(
                                    titulaire_id=participant.titulaire_id,
                                    race_id=finale_race_a.id,
                                    couloir_id=couloirs_a[index_couloir_a].id,
                                ))
                                index_couloir_a += 1
                            else:
                                db.session.add(Participant_race(
                                    titulaire_id=participant.titulaire_id,
                                    race_id=finale_race_b.id,
                                    couloir_id=couloirs_b[index_couloir_b].id,
                                ))
                                index_couloir_b += 1
                    ...
                    quart_races = Race.query.filter_by(categorie_id=categorie.id, race_type_id=phase_quart.id).all()
                    finale_race_c = Race.query.filter_by(categorie_id=categorie.id, race_type_id=phase_finale.id, name="C").first()
                    for i in range(2) :
                        db.session.add(Manche(race_id=finale_race_c.id))
                    db.session.commit()
